...
from tenant_state import files_dir, tenant_name
...
def _resolve(path):
    ...
    p = os.path.abspath(s if os.path.isabs(s) else os.path.join(scoped or BASE, s))
...
def _allowed_roots():
    ...
    elif tenant_name() == "local":
        roots.append(BASE)
    try:
        import history
        for pr in history.list_projects():
            p = pr.get("path", "")
