...
def _resolve_src(path):
    ...
    raw = str(path)
    p = os.path.abspath(raw if os.path.isabs(raw) else os.path.join(scoped or WORKSPACE, raw))
    roots = [files_dir(), os.path.join(BASE, "plugins"), os.path.join(BASE, "skills")]
    if scoped:
        roots.insert(0, os.path.abspath(scoped))
    else:
    ...
    try:
        ...
        for project in history.list_projects():
            if project.get("path"):
                roots.append(os.path.abspath(project["path"]))
    except Exception:
        pass
    for r in roots:
...
def write_file_with_link(args):
    ...
    scoped_workspace = current_workspace.get() or WORKSPACE
    p = _resolve_src(path) if os.path.isabs(path) else os.path.join(scoped_workspace, _safe(path))
