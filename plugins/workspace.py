...
DATA_DIR = os.environ.get("WENMO_DATA_DIR") or BASE
WORKSPACE = os.path.join(DATA_DIR, "workspace") if os.environ.get("WENMO_DATA_DIR") else BASE
PROJECTS_FILE = os.path.join(DATA_DIR, "projects.json")
os.makedirs(WORKSPACE, exist_ok=True)
MAX_FILE_SIZE = 1024 * 1024  # 1MB: prevents accidental prompt/memory blow-ups


def _allowed_roots():
    scoped = current_workspace.get()
    if scoped:
        return [os.path.abspath(scoped)]
    roots = [
        os.path.abspath(WORKSPACE),   # 项目根目录（含 src/ docs/ plugins/ skills/ 等）
    ]
    try:
        import history
        for p in history.list_projects():
            if p.get("path"):
...
def _resolve(path):
    scoped = current_workspace.get()
    raw = str(path or "")
    base = os.path.abspath(scoped or WORKSPACE)
    p = os.path.abspath(raw if os.path.isabs(raw) else os.path.join(base, raw))
...
def list_files(args):
    path = args.get("path") or current_workspace.get() or WORKSPACE
...
def search_in_directory(args):
    ...
    path = str(args.get("path", "")).strip() or current_workspace.get() or WORKSPACE
