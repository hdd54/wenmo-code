...
from execution_context import current_task_id, current_tenant, current_workspace
from task_workspace import TaskWorkspaceManager

BASE = os.path.dirname(os.path.abspath(__file__))
DB_PATH = os.path.join(BASE, "tasks.db")
WORKTREE_ROOT = os.path.join(os.environ.get("WENMO_DATA_DIR") or BASE, "task-worktrees")
WORKSPACES = TaskWorkspaceManager(WORKTREE_ROOT)

MAX_LOOPS = 40           # 后台任务必须有硬上限，避免失控循环和意外计费
MAX_TOOL_RESULT = 100_000  # 保留足够诊断信息，同时阻止超大工具结果拖垮上下文
...
class TaskStore:
    def __init__(self, db=DB_PATH):
        ...
        self._conn.execute("""CREATE TABLE IF NOT EXISTS tasks (
            ...
            project TEXT, repo_path TEXT, worktree_path TEXT, branch TEXT,
            tenant TEXT DEFAULT 'local', sandbox_mode TEXT DEFAULT 'required',
        ...
        for name, declaration in migrations.items():
            if name not in columns:
                self._conn.execute("ALTER TABLE tasks ADD COLUMN %s %s" % (name, declaration))
        self._conn.commit()

    def submit(self, task):
        tid = task["id"]
        self._conn.execute(
            "INSERT OR REPLACE INTO tasks (id,status,priority,provider,model,messages,online,reasoning,project,repo_path,worktree_path,branch,tenant,sandbox_mode,created,updated) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
            (tid, "pending", task.get("priority", 0), task["provider"], task.get("model", ""),
             json.dumps(task.get("messages", []), ensure_ascii=False),
             1 if task.get("online") else 0, task.get("reasoning", ""), task.get("project", "default"),
             task.get("repo_path", ""), task.get("worktree_path", ""), task.get("branch", ""),
             task.get("tenant", "local"), task.get("sandbox_mode", "required"),
             time.time(), time.time()))
        self._conn.commit()
        return tid
    ...
    def get(self, tid, tenant=None):
        if tenant is None:
            row = self._conn.execute("SELECT * FROM tasks WHERE id=?", (tid,)).fetchone()
        else:
            row = self._conn.execute(
                "SELECT * FROM tasks WHERE id=? AND COALESCE(tenant, 'local')=?", (tid, tenant)).fetchone()
        return self._row_to_task(row) if row else None

    def list(self, limit=50, tenant=None):
        if tenant is None:
            rows = self._conn.execute(
                "SELECT * FROM tasks ORDER BY created DESC LIMIT ?", (limit,)).fetchall()
        else:
            rows = self._conn.execute(
                "SELECT * FROM tasks WHERE COALESCE(tenant, 'local')=? ORDER BY created DESC LIMIT ?",
...
async def run_task(task):
    ...
    tools, flatten_map = g._flatten_tools(tools)
    last_user = ""
    ...
    tool_count = 0
    workspace_token = current_workspace.set(task.get("worktree_path") or task.get("repo_path") or "")
    task_token = current_task_id.set(str(task.get("id") or ""))
    tenant_token = current_tenant.set(str(task.get("tenant") or "local"))
    try:
