...
from extension_packages import mcp_servers as extension_mcp_servers
...
LOCAL_CONFIG_FILE = os.path.join(os.path.dirname(CONFIG_FILE), "mcp.local.json")
...
def _migrate_inline_secrets(config_path=CONFIG_FILE, local_path=LOCAL_CONFIG_FILE):
    """Move plaintext MCP env secrets into the ignored local overlay, preserving behavior."""
    try:
        with _CONFIG_LOCK:
            ...
            try:
                ...
                local = {"servers": {}}
            changed = False
            for name, entry in (base.get("servers") or {}).items():
                env = entry.get("env") or {}
                for key, value in list(env.items()):
                    key_lower = str(key).lower()
                    value_text = str(value or "").strip()
                    sensitive = bool(re.search(
                        r"(^|[_-])(api[_-]?key|key|token|secret|password|credential)([_-]|$)", key_lower))
                    placeholder = value_text.startswith(("${", "env:", "%"))
                    if not (sensitive and value_text and not placeholder):
                        continue
                    local_entry = local.setdefault("servers", {}).setdefault(name, {})
                    local_entry.setdefault("env", {})[key] = value
                    env[key] = ""
                    changed = True
            if changed:
                _atomic_json_write(local_path, local)
                _atomic_json_write(config_path, base)
            return changed
    except Exception:
        return False


def _merge_server_config(base, local):
    merged = dict(base or {})
    merged.update(local or {})
    merged["env"] = {**((base or {}).get("env") or {}), **((local or {}).get("env") or {})}
    return merged


def _app_dir():
    """程序目录：打包版 = exe 所在目录；开发版 = 本文件目录"""
...
def _interpolate(value, env):
    ...
    内置占位符：${APP_DIR}（程序目录）、${DATA_DIR}（数据目录）、${PYTHON}（python 解释器）。"""
    extra = {
        "APP_DIR": _app_dir(),
        "DATA_DIR": os.environ.get("WENMO_DATA_DIR") or BASE_DIR,
        "RES_DIR": os.environ.get("WENMO_RES_DIR") or BASE_DIR,
        "PYTHON": sys.executable if not getattr(sys, "frozen", False) else "python",
    }
    # A packaged client resolves only built-in paths and values explicitly
    # stored in its own config. It must not inherit developer/build-machine
    # tokens or private environment variables.
    _env = {} if getattr(sys, "frozen", False) else dict(os.environ)
    _env.update(extra)
    _env.update(env or {})
    env = _env
...
def load_mcp_config():
    ...
    try:
        ...
        try:
            ...
            local_data = {}
        combined_servers = dict(data.get("servers") or {})
        for _package_name, _package_entry in extension_mcp_servers().items():
            combined_servers.setdefault(_package_name, _package_entry)
        for _name, _local_entry in (local_data.get("servers") or {}).items():
            combined_servers[_name] = _merge_server_config(combined_servers.get(_name), _local_entry)
        for _n2, _e2 in list(combined_servers.items())[:32]:
            if not isinstance(_e2, dict) or _e2.get("enabled") is False:
                continue
            servers.setdefault(_n2, {
                "command": _interpolate(_e2.get("command") or [], {}),
                "env": _interpolate(_e2.get("env") or {}, {}),
            })
        # 内置 server 回退（内置集成）：打包版 command 为 [问墨.exe --mcp-server X]；
        # 开发版没有 问墨.exe → 回退为用当前 python 运行对应脚本
        _BUILTIN_SCRIPTS = {
            "demo": "mcp_demo_server.py",
            "websearch": "websearch_mcp_server.py",
            "file-mcp": "file_mcp_server.py",
            "ppt-pipeline": "ppt_pipeline_mcp_server.py",
        }
        for _n2, _cfg2 in list(servers.items()):
            _cmd2 = _cfg2.get("command") or []
            if _cmd2 and not os.path.isfile(str(_cmd2[0])) and _n2 in _BUILTIN_SCRIPTS:
                _sp = os.path.join(BASE_DIR, _BUILTIN_SCRIPTS[_n2])
                if os.path.isfile(_sp):
                    _cfg2["command"] = [sys.executable, _sp]
        # Development builds automatically reuse the developer's OpenCode MCP
        # registry. Packaged clients must never import machine-private paths,
        # commands or credentials from a developer profile.
        import_path = data.get("importFromOpencode", "")
        if not getattr(sys, "frozen", False) and not import_path:
            import_path = os.path.expandvars(r"%USERPROFILE%\.config\opencode\opencode.json")
        if getattr(sys, "frozen", False):
            import_path = ""
