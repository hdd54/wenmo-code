 ...
 *   3. 停止：AbortController 中断流，已生成的部分保留。
...
function postProcess(container) {
  ...
  container.querySelectorAll("a[href*='/files/']").forEach(function (a) {
    ...
    dl.addEventListener("click", function (ev) {
      ...
      tmp.href = a.href;
      // 用 URL 里的真实文件名作为下载名，避免浏览器按 Content-Type 存成 .txt
      try {
        tmp.download = decodeURIComponent(new URL(a.href).pathname.split("/").pop() || "");
      } catch (err) {
...
function renderFilePreview(name) {
  ...
  if (ext === "pdf") {
    ...
    // PPT：Office 渲染预览（转 PDF 用查看器）；失败回退幻灯片播放器
    officePdfPreview(name, function () { renderPptPreview(name); });
  } else if (ext === "docx" || ext === "doc") {
    // Word：Office 渲染预览（转 PDF 用查看器）；失败回退文档预览器
    officePdfPreview(name, function () { renderDocxPreview(name); });
  } else if (ext === "xlsx" || ext === "xls") {
    // Excel：Office 渲染预览（转 PDF 用查看器）；失败回退表格预览
    officePdfPreview(name, function () { renderXlsxPreview(name); });
...
function officePdfPreview(name, fallback) {
  var body = document.getElementById("file-panel-body");
  if (!body) return;
  body.innerHTML = "";
  // 转换中提示（Office 首次启动需几秒）
  var hint = document.createElement("div");
  hint.className = "steps-empty";
  hint.textContent = "正在用 Office 渲染预览（首次转换需几秒）…";
  ...
  f.onload = function () {
    if (done) return;
    // 转换成功（iframe 加载了 PDF）→ 显示 iframe
    if (f.contentDocument && f.contentDocument.body && f.contentDocument.body.childElementCount > 0) {
      done = true;
      hint.remove();
      f.style.display = "";
      return;
  ...
  // Office 转换较慢（首次 10-40 秒）：只对明确失败回退，不因慢而误回退
  var triedFallback = false;
  setTimeout(function () {
    if (done) return;
    try {
      if (f.contentDocument && f.contentDocument.body && f.contentDocument.body.children.length) {
        done = true; hint.remove(); f.style.display = "";
        return;
      }
    } catch (e) { /* 跨域不可读：视为已加载（PDF 查看器） */ done = true; hint.remove(); f.style.display = ""; return; }
    // 45 秒仍无内容 → 回退文字预览
    if (!triedFallback) {
      triedFallback = true;
      hint.textContent = "Office 渲染超时，切换为文字预览…";
      setTimeout(function () { if (!done && typeof fallback === "function") fallback(); }, 300);
    }
  }, 45000);
}
...
function downloadCurrentFile() {
  ...
  var a = document.createElement("a");
  a.href = "/files/" + encodeURIComponent(filePanelName);
  a.download = filePanelName;
  document.body.appendChild(a);
  a.click();
  a.remove();
}
...
function updateSendBtn() {
  ...
  // 只有用户在输入时才显示发送箭头；生成中（未输入）显示停止方块
  var showStop = state.streaming && !hasText;
...
function showTaskStatus(text, count) {
  ...
  bar.hidden = false;
  var t = document.getElementById("ts-text");
  if (t) {
    ...
    t.textContent = curStepText || text || "处理中…";
    // 缩短显示（状态栏单行，过长截断加省略号）
    if (t.textContent.length > 80) t.textContent = t.textContent.slice(0, 77) + "…";
  }
  var s = document.getElementById("ts-step");
  if (s) {
...
function renderAllMessages() {
  ...
  // 长对话（>80 条）：分批渲染（每帧 60 条），切换不卡顿
  // 注意：不在批次开始前滚动（DOM 高度未定 → 滚到半空）；只在全部批次完成后滚到底
  var cur = 0;
  (function nextBatch() {
    var frag = document.createDocumentFragment();
    var end = Math.min(cur + 60, msgs.length);
    try {
      renderInto(frag, cur, end);
      cur = end;
      els.messages.appendChild(frag);
    } catch (e) {
      // 批次异常：跳过该批继续（防止 rAF 链中断 → 对话隐藏）
      cur = end;
    }
    if (cur < msgs.length) {
      buildMsgAnchors();              // 分批渲染期间也逐步刷新锚点（用户可提前定位）
      requestAnimationFrame(nextBatch);
    } else {
      scrollIfNearBottom();   // 全部渲染完成 → 此时滚动位置才正确
      buildMsgAnchors();
    }
  })();
}
...
/**
 * 收尾：ok / stop / error。
 * ok 与 stop 都把已生成内容存进会话历史（多轮对话需要）；error 不存，只提示。
 */
function finishStream(kind, errMsg, cid) {
  // 多流收尾：用本流的 ctx（cid 从 streamCtxs 取；后台流收尾不污染当前 UI 的全局状态）
  var ctx = (cid && streamCtxs[cid]) || streamCtx || null;
  if (!ctx) return;
  if (ctx.finished) return;                        // 防止重复收尾
  stopStreamingIndicator(ctx.cid);                 // 移除本流的历史项生成动画
  ctx.finished = true;
  var detached = ctx.cid && ctx.cid !== state.conversationId;
  // 后台流（detached）收尾：不动当前 UI 的 streaming/controller/sendingLock/状态栏
  if (!detached) {
    hideTaskStatus();
    state.streaming = false;
    state.controller = null;
    state.sendingLock = false;                     // 解锁发送（防连点锁定解除）
    setCaret(false);
    clearWaitHint();
  }
  if (thinkingEl) { thinkingEl.remove(); thinkingEl = null; }   // 移除“正在思考”指示器

  if (kind === "error") {
    if (!detached && aiMsgEl && aiMsgEl.isConnected && !acc.trim() && !thinkAcc.trim()) {
      aiMsgEl.remove();                             // 一个字都没出，别留空框
    }
    if (detached) {
      // 后台对话出错：不打扰当前界面，仅刷新历史列表
      renderHistoryList();
    } else {
      finalizePlan();
    ...
    if (acc.trim() || (ctx.flow && ctx.flow.acc.trim())) {
      // 历史完整保存：assistant 消息附带思考过程与工具调用记录
      // 用本流的独立缓冲（后台流防串台：不取全局 acc 而是本流 flowState）
      var fs = ctx.flow || { acc: acc, thinkAcc: thinkAcc, toolLog: toolLog, timeline: [] };
      var am = { role: "assistant", content: fs.acc };
      // 本次生成耗时（秒）→ 历史渲染显示"用时 X"
      ...
      // 交错时间线（think/tool/text 按序）→ 历史渲染还原实时对话形态（而非拼接成一坨）
      if (fs.timeline && fs.timeline.length) am.timeline = fs.timeline.slice();
      if (detached) {
        // 把结果写回原对话历史（不污染当前对话）
        // 关键：若用户已切回该对话并追加了消息（state.messages 比流快照新），
        // 用最新 state.messages 而非 ctx.messages 快照，避免覆盖丢消息（"切换导致对话中断"根因）
        var full;
        if (state.conversationId === ctx.cid && state.messages && state.messages.length >= ctx.messages.length) {
          full = state.messages.slice();
        } else {
          full = ctx.messages.slice();
        }
        full.push(am);
        fetch("/api/history", {
          method: "POST",
        ...
        // 对话 A 后台完成：若它有排队消息 → 后台续答（用户输入不消失，存回 A 历史）
        var aq = state.sendQueues[ctx.cid];
        if (aq && aq.length && ctx.cid) {
          var nxt = aq.shift();
          saveQueue(ctx.cid, aq);   // 持久化
          logQueueEvent(ctx.cid, { type: "dequeue_bg", msg: nxt.text, id: nxt.id });
          backgroundContinue(ctx, nxt.text);
        }
      } else {
        state.messages.push(am);
        renderCurrentContent();                       // 最终定稿渲染
        if (aiMsgEl) {                                // 给刚生成的 AI 消息挂操作栏 + 生成速度
          aiMsgEl.dataset.idx = state.messages.length - 1;
          var speedSecs = (Date.now() - streamStartTs) / 1000;
          var spd = document.createElement("span");
          spd.className = "tok-speed";
          // 显示：生成速度 + 本次对话耗时（对标 opencode 的耗时反馈）
          var timeStr = speedSecs >= 60
            ? Math.floor(speedSecs / 60) + "分" + Math.round(speedSecs % 60) + "秒"
            : Math.round(speedSecs) + "秒";
          if (streamOutTokens > 0 && speedSecs > 0.3) {
            spd.textContent = "⚡ " + (streamOutTokens / speedSecs).toFixed(1) + " tok/s · 用时 " + timeStr;
          } else {
            spd.textContent = "用时 " + timeStr;
          }
          aiMsgEl.appendChild(spd);
          aiMsgEl.appendChild(buildMsgActions({
            role: "assistant",
            idx: state.messages.length - 1,
            getText: function () { return acc; }
          }));
        }
        // 回答完成弹窗（对标 opencode "Agent ready for input"）：3 秒自动消失
        finalizePlan();   // 当前对话完成：规划未归档则收尾（防后台对话污染步骤历史）
        showDoneToast("AI 回答完成，可以继续输入");
      }
    } else if (!detached && aiMsgEl && aiMsgEl.isConnected) {      // 空回复不悄悄消失：正常结束提示重试，主动停止提示已停止
      var note = document.createElement("div");
      note.className = "minor-note";
      if (thinkAcc.trim() && !acc.trim()) {
        note.textContent = kind === "stop" ? "（已停止生成）" : "（模型仅返回思考过程，请重试）";
      } else {
        note.textContent = kind === "stop" ? "（已停止生成）" : "（模型未返回内容，请重试）";
      }
      aiMsgEl.appendChild(note);
    }
    if (!detached) {
      setStatus("idle");
      finalizeStepThink("思考完成");                    // 收尾最后一个步骤的思考
      if (kind !== "error" && state.messages.length > 0) {
        saveHistory();                                  // 自动保存历史
        flashHistoryDotFor(state.conversationId);       // 历史列表当前项闪亮点提醒
      }
    }
  }

  setSendBtn("send");
  // 无论正常/停止/出错：只要有过内容就保存（模型回答到哪保存到哪，不丢对话）
  if (state.messages.length > 0) {
    if (!detached) {
      saveHistory();
      if (kind === "error") renderHistoryList();
    } else {
      renderHistoryList();
    }
  }
  // 从多流 Map 移除本流；若本流是当前引用则清空
  if (ctx.cid && streamCtxs[ctx.cid] === ctx) delete streamCtxs[ctx.cid];
  if (streamCtx === ctx) streamCtx = null;
  // 发送队列：有排队的消息 → 自动按顺序发下一条（对标 codex）
  if (kind !== "error" && !detached) setTimeout(dequeueNext, 300);
}
...
function startToolLine(name, args) {
  ...
  // 智能体代理调用 → 青色高亮（功能分色）
  if (/agent|delegate/i.test(name)) line.classList.add("agent");
...
function sendMessage() {
  ...
  if (state.attachments.length > 0) {
    ...
    Promise.all(attachPromises).then(function () {
      ...
      renderUserMessage(note, state.messages.length - 1);
      clearAttachments();
      els.input.value = "";
      autoResizeInput();
      updateSendBtn();
  ...
  fetch("/api/chat", {
    ...
    function pump() {
      return reader.read().then(function (r) {
        if (r.done) {
          if (acc.trim()) contentEl.innerHTML = renderMarkdown(acc);
          else contentEl.textContent = "（对比模型未返回内容）";
          return;
        }
        acc += decoder.decode(r.value, { stream: true });
        // 增量渲染：保留最后一段不完整 JSON 到下一帧
        var lines = acc.split("\n");
        if (lines.length > 2) {
          var done = lines.slice(0, -1).join("\n");
          var textOut = "";
          done.split("\n").forEach(function (ln) {
            if (ln.indexOf("data: ") === 0) {
              try {
                var evt = JSON.parse(ln.slice(6));
                if (typeof evt.delta === "string") textOut += evt.delta;
              } catch (e) { /* 忽略不完整行 */ }
            }
          });
          if (textOut.trim()) contentEl.innerHTML = renderMarkdown(textOut);
        }
        return pump();
      });
    }
...
/** 停止生成：abort 掉 fetch，已生成内容保留 */
function stopStreaming() {
  if (!state.streaming) return;
  if (state.controller) state.controller.abort();
  // abort 的落点在 streamChat 的 catch（AbortError）里收尾
}
...
async function streamChat(opts) {
    ...
    try {
      ...
      if (seed && seed.id) {
        ...
        if (streamCtx) {
          ...
          var oldKey = streamCtx.cid || "";
          streamCtx.cid = seed.id;
          if (oldKey && oldKey !== seed.id && streamCtxs[oldKey] === streamCtx) {
            delete streamCtxs[oldKey];
          }
          streamCtxs[seed.id] = streamCtx;   // 用真实 id 重新挂载（flow 才能被 streamChat 找到）
        }
  ...
  var flowCid = state.conversationId || "";   // 本流发起时的对话 id（切走后不变，收尾用正确的流上下文）
  ...
  try {
    ...
    var outcome = await readEventStream(res.body, controller.signal, {
      onThink: function (fragment) {
        ...
        scrollIfNearBottom();   // 思考也跟随最新（对标 opencode）
      },
      onDelta: function (delta) {
        ...
        scrollIfNearBottom();                       // 跟随最新输出（对标 opencode）
      },
      onTool: function (info) {
        ...
        if (!isCurrentFlow()) {
          ...
          if (info.phase === "done") {
            flowState.toolLog.push({ name: info.name, args: (info.args || "").slice(0, 120),
              done: !!info.ok, result: (info.result || "").slice(0, 120),
              diff: info.diff ? info.diff.slice(0, 6000) : undefined });
            flowState.timeline.push({ type: "tool", name: info.name,
              args: (info.args || "").slice(0, 120), done: !!info.ok,
              result: (info.result || "").slice(0, 120),
              diff: info.diff ? info.diff.slice(0, 6000) : undefined });
            _tlPush("tool");
          }
          return;
        ...
        if (info.phase === "start") {
          ...
          if (tIdx >= 0) {
            ...
            toolLog[tIdx].result = (info.result || "").slice(0, 120);
            if (info.diff) toolLog[tIdx].diff = info.diff.slice(0, 6000);
          } else {
            toolLog.push({ name: info.name, args: (info.args || "").slice(0, 120), done: info.ok, result: (info.result || "").slice(0, 120), diff: info.diff ? info.diff.slice(0, 6000) : undefined });
          }
          ...
          if (fIdx >= 0) {
            ...
            flowState.toolLog[fIdx].result = (info.result || "").slice(0, 120);
            if (info.diff) flowState.toolLog[fIdx].diff = info.diff.slice(0, 6000);
          } else {
            flowState.toolLog.push({ name: info.name, args: (info.args || "").slice(0, 120), done: info.ok, result: (info.result || "").slice(0, 120), diff: info.diff ? info.diff.slice(0, 6000) : undefined });
          }
      ...
      onUsage: function (usage) {
        ...
        var cost = calcCost(usage.input || 0, usage.output || 0, usage.cached || 0);
...
function renderGeneralPanel() {
  ...
  fetch("/api/settings/permissions").then(function (r) { return r.json(); }).then(function (p) {
    if (pw) pw.value = p.write_files || "allow";
    if (pc) pc.value = p.run_command || "ask";
  }).catch(function () { /* 忽略 */ });
  var savePerm = function () {
    fetch("/api/settings/permissions", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ write_files: pw ? pw.value : "allow", run_command: pc ? pc.value : "ask" })
    }).catch(function () { /* 忽略 */ });
  };
  if (pw) pw.addEventListener("change", savePerm);
  if (pc) pc.addEventListener("change", savePerm);
  initBillingModal();
}

/* ===========================================================================
 * 计费统计弹窗（设置 → 通用 → 总计费）
 * 视图1：对话明细（点击可进入对话）；视图2：供应商分模型条形图 + 时间维度 + 每日趋势
 * ========================================================================= */
var billingData = null;      // 缓存 /api/billing 数据
var billingWindow = "all";   // 当前时间维度
var billingTab = "conv";     // 当前视图

function initBillingModal() {
  var btn = document.getElementById("gen-billing-btn");
  if (btn && !btn.dataset.filled) {
    btn.dataset.filled = "1";
    btn.addEventListener("click", openBillingModal);
  }
  var modal = document.getElementById("billing-modal");
  if (!modal) return;
  modal.querySelectorAll("[data-billing-close]").forEach(function (el) {
    el.addEventListener("click", function () { modal.hidden = true; });
  });
  // 时间窗口切换
  modal.querySelectorAll(".bw-btn").forEach(function (b) {
    b.addEventListener("click", function () {
      modal.querySelectorAll(".bw-btn").forEach(function (x) { x.classList.remove("active"); });
      b.classList.add("active");
      billingWindow = b.dataset.bw;
      renderBilling();
    });
  });
  // 视图切换
  modal.querySelectorAll(".bt-tab").forEach(function (b) {
    b.addEventListener("click", function () {
      modal.querySelectorAll(".bt-tab").forEach(function (x) { x.classList.remove("active"); });
      b.classList.add("active");
      billingTab = b.dataset.bt;
      var cv = document.getElementById("billing-conv");
      var mv = document.getElementById("billing-model");
      if (cv) cv.hidden = billingTab !== "conv";
      if (mv) mv.hidden = billingTab !== "model";
      renderBilling();
    });
  });
  // Esc 关闭
...
/** 按当前时间窗口过滤对话 */
function billingConvInWindow(c) {
  if (billingWindow === "all" || !c.updated) return true;
  var day = 86400, now = Date.now() / 1000;
  var days = billingWindow === "today" ? 1 : (billingWindow === "week" ? 7 : (billingWindow === "month" ? 30 : 365));
  return c.updated >= now - days * day;
}

function renderBilling() {
  if (!billingData) return;
  // 总计费 = 当前时间窗口内对话的费用之和（不是全历史总额）
  var convs = (billingData.by_conv || []).filter(billingConvInWindow);
  var winTotal = convs.reduce(function (acc, c) { return acc + (c.cost || 0); }, 0);
  document.getElementById("billing-total").textContent = "¥" + winTotal.toFixed(2);
  if (billingTab === "conv") renderBillingConvs();
  else renderBillingModels();
}

function renderBillingConvs() {
  ...
  convs.forEach(function (c) {
    ...
    var t = document.createElement("div");
    t.className = "billing-conv-title";
    t.textContent = c.title || "未命名对话";
    var meta = document.createElement("div");
    meta.className = "billing-conv-meta";
    var when = c.updated ? fmtTime(Math.floor(c.updated)) : "";
    meta.textContent = (c.provider || "") + " · " + (c.model || "") + (when ? " · " + when : "")
      + (c.tokens ? " · " + (c.tokens / 1000).toFixed(0) + "k tokens" : "");
    info.appendChild(t);
    ...
    row.addEventListener("click", function () {
      ...
      closeSettings();
      loadConversation(c.cid);
    });
    list.appendChild(row);
  });
}

function renderBillingModels() {
  ...
  // 分模型条形图
  var models = (billingData.by_model || []).filter(function (m) {
    return m.cost > 0 && billingConvInWindow({ updated: billingWindow === "all" ? 1 : 1 });
  });
  // 按时间窗口过滤：by_model 无时间，用窗口内对话重算
  if (billingWindow !== "all") {
    var convs = (billingData.by_conv || []).filter(billingConvInWindow);
    var acc = {};
    convs.forEach(function (c) {
      var k = c.provider + "/" + c.model;
      if (!acc[k]) acc[k] = { provider: c.provider, model: c.model, cost: 0, convs: 0 };
      acc[k].cost += c.cost;
      acc[k].convs += 1;
    });
    models = Object.keys(acc).map(function (k) { return acc[k]; })
  ...
  if (!models.length) {
    ...
    var maxCost = Math.max.apply(null, models.map(function (m) { return m.cost; })) || 1;
    models.forEach(function (m) {
      var row = document.createElement("div");
      row.className = "billing-bar-row";
      var label = document.createElement("div");
      label.className = "billing-bar-label";
      label.textContent = (m.provider || "?") + " / " + (m.model || "?");
      ...
      val.className = "billing-bar-val";
      val.textContent = "¥" + m.cost.toFixed(2);
  ...
  // 每日趋势条形图
  daily.innerHTML = "";
  var days = billingData.daily || [];
  if (!days.length) {
    daily.innerHTML = '<div class="billing-empty">暂无每日费用数据</div>';
    return;
  }
  var maxD = Math.max.apply(null, days.map(function (d) { return d.cost; })) || 1;
  var dRow = document.createElement("div");
  dRow.className = "billing-daily-row";
  days.forEach(function (d) {
    var cell = document.createElement("div");
    cell.className = "billing-daily-cell";
    cell.title = d.date + ": ¥" + d.cost.toFixed(4);
    var fill = document.createElement("div");
    fill.className = "billing-daily-fill";
    fill.style.height = Math.max(3, (d.cost / maxD) * 100) + "%";
    cell.appendChild(fill);
    var lbl = document.createElement("div");
    lbl.className = "billing-daily-lbl";
    lbl.textContent = d.date;
    cell.appendChild(lbl);
    dRow.appendChild(cell);
  });
  daily.appendChild(dRow);
}
...
function bindEvents() {
  ...
  // 联网开关（发送时带 online=true，服务器优先保留搜索工具）
  var onlineBtn = document.getElementById("online-btn");
  if (onlineBtn) {
    onlineBtn.addEventListener("click", function () {
      state.online = !state.online;
      onlineBtn.setAttribute("aria-pressed", String(state.online));
      onlineBtn.classList.toggle("on", state.online);
      onlineBtn.title = state.online ? "已开启联网搜索（点击关闭）" : "联网搜索：开启后 AI 可用搜索工具获取实时信息";
    });
  }
  ...
  els.messages.addEventListener("drop", function (e) {
    ...
    var jobs = files.map(function (f) { return fileToDataURL(f, 1024); });
    ...
    b.addEventListener("click", function () {
      ...
      if (p.launch_cmd && p.launch_cmd.trim()) {
        fetch("/api/launch", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ cmd: p.launch_cmd.trim(), cwd: p.path || "" })
        }).catch(function () { /* 忽略 */ });
      }
...
function openProjectRailMenu(anchor) {
  ...
  folderItem.className = "command-menu-item";
  folderItem.innerHTML = '<span class="cm-cmd">打开项目文件夹</span>';
  folderItem.addEventListener("click", function () {
    menu.remove();
    fetch("/api/projects/" + state.project + "/open-folder", { method: "POST" }).catch(function () {});
  });
...
function addProjectPrompt() {
  ...
  if (name === null) return;
  // 弹出原生 Windows 文件夹选择器（由服务器代劳，浏览器拿不到本地路径）
...
function updateTokenUsage() {
  ...
  if (costEl) {
    ...
    costEl.title = "当前对话累计费用（估算，按供应商模型价格表）"
      + "\n模型: " + pmodel
...
function openHistoryCtxMenu(x, y, c) {
  ...
  var actions = [
    ...
    { label: "重命名", fn: function () {
        ...
        }).then(function () { renderHistoryList(); }).catch(function () {});
      } },
...
/** 新对话：清空当前会话 */
function newChat() {
  // 新对话不打断后台生成（opencode 式）：原对话的流继续跑，结果存回原对话历史
  dismissAskOnSwitch();
  saveInputDraft();                                  // 保存当前对话的输入草稿                              // ask 弹窗不阻碍新对话
  state.streaming = false;                           // 脱离 UI 不 abort（流后台继续）
...
function loadConversation(cid) {
  // 切换对话不打断后台生成（opencode 式）：正在进行的流继续跑，结果自动存回原对话历史；
  // 切回时如果还在生成则显示提示。
  dismissAskOnSwitch();                              // ask 弹窗不阻碍切换对话
  saveInputDraft();                                  // 切走前：保存当前对话的输入草稿
  // 脱离 UI 不 abort：原对话流后台继续（streamCtx 多流管理，收尾存回各自历史）
  state.streaming = false;
  ...
  if (cached) {
    ...
    updateTokenUsage();
    clearAttachments();
    renderAllMessages();                             // 渲染完成即滚到底（scrollIfNearBottom），不提前滚
    restoreStreamUI();
  } else {
  ...
  if (extra) {
    ...
    if (extra.timeline && extra.timeline.length) {
      ...
      extra.timeline.forEach(function (tl, ti) {
        if (tl.type === "think") {
          ...
          if (/agent|delegate/i.test(tl.name)) tl2.classList.add("agent");
    ...
    if (extra.tools && extra.tools.length) {
      ...
      extra.tools.forEach(function (t) {
        ...
        if (/agent|delegate/i.test(t.name)) line.classList.add("agent");
...
function saveHistory() {
  ...
  fetch("/api/history", {
    ...
    body: JSON.stringify({
      ...
      provider: state.activeProvider ? state.activeProvider.key : "",
      model: state.activeProvider ? state.activeProvider.model : "",
      usage: { input: state.totalInput, output: state.totalOutput, output_formal: state.totalOutputFormal, cached: state.totalCached, context: state.lastContext, last_input: state.lastInput, last_cached: state.lastCached, cost: state.totalCost, cost_est: state.costIsEst }
    })
    })
    .then(function (res) { return res.json(); })
    .then(function (data) {
  ...
  win.innerHTML =
    ...
    (inlineClock
      ? '<div class="clock-inline">' +
          ...
          '</div>' +
        '</div>'
      : '<iframe class="app-window-frame" allowtransparency="true" style="background:transparent" src="/apps/' + encodeURIComponent(a.name) + '/index.html?theme=' + encodeURIComponent(els.root.getAttribute("data-theme") || "dark") + '&ts=' + Date.now() + '" title="小应用"></iframe>') +
...
function applySidebarCollapsed(collapsed) {
  ...
  setPref("sidebar_collapsed", collapsed ? "1" : "0");
}
...
function initSidebarCollapse() {
  ...
  if (getPref("sidebar_collapsed", "0") === "1") applySidebarCollapsed(true);
}
